...
HTML-код `<iframe>` генерируется динамически на стороне Партнера, атрибут `src` содержит уникальный для каждого пользователя URL. Атрибут `seamless` обеспечивает корректную работу в некоторых браузерах. Следует указать атрибут `name` в случае несколькольких iframe-ов на странице, он должен содержать уникальное значение, например внунтренн.
